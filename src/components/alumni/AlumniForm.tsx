...
import { MENTOR_CATEGORIES, MENTOR_CATEGORY_LABELS, type Alumni } from "@/mocks";
...
const schema = z.object({
  ...
  linkedin_url: z.string().refine((v) => !v || v.startsWith("https://linkedin.com/"), "LinkedIn URL must start with https://linkedin.com/").optional().or(z.literal("")),
...
export function AlumniForm({ mode, initial }: { mode: "create" | "edit"; initial?: Alumni }) {
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-4 pb-24">
      <Section title="Basic Info">
        ...
        <Field label="LinkedIn URL" error={errors.linkedin_url?.message}><Input placeholder="https://linkedin.com/in/…" {...register("linkedin_url")} /></Field>
      </Section>

      <Section title="Academic">
        <Field label="Graduation year" error={errors.graduation_year?.message}><Input type="number" {...register("graduation_year")} /></Field>
        <Field label="Degree program"><Input {...register("degree_program")} /></Field>
      </Section>
      ...
      <Section title="Skills & Mentorship">
        <Field label="Technical skills">
          ...
          <div className="flex flex-wrap gap-1.5 mt-2">
            {skills.map((s) => (
              <Badge key={s} variant="secondary" className="gap-1">{s}<button type="button" onClick={() => setSkills(skills.filter((x) => x !== s))}><X className="size-3" /></button></Badge>
            ))}
          </div>
        </Field>
        <Controller control={control} name="mentorship_interest" render={({ field }) => (
          <Field label="Mentorship interest">
            <div className="flex items-center gap-3"><Switch checked={field.value} onCheckedChange={field.onChange} /><span className="text-sm text-muted-foreground">Available to mentor current students</span></div>
          </Field>
        )} />
        {mentorOn && (
          <Controller control={control} name="mentorship_categories" render={({ field }) => (
            <Field label="Mentorship categories">
              <div className="grid grid-cols-2 gap-2">
                {MENTOR_CATEGORIES.map((c) => (
                  <label key={c} className="flex items-center gap-2 text-sm cursor-pointer">
                    <Checkbox checked={field.value.includes(c)} onCheckedChange={(v) => field.onChange(v ? [...field.value, c] : field.value.filter((x) => x !== c))} />
      ...
      <Section title="Notes">
        <Field label="Notes"><Textarea rows={4} {...register("notes")} /></Field>
      </Section>

      <div className="fixed bottom-0 left-60 right-0 bg-background/95 backdrop-blur border-t p-4 flex justify-end gap-2 z-20">
        <Button type="button" variant="outline" onClick={() => navigate({ to: "/alumni" })}>Cancel</Button>
        <Button type="submit">{mode === "create" ? "Create alumni" : "Save changes"}</Button>
      </div>
    </form>
  );
}
